...
module Make (Ord : OrderedType) = struct
  ...
  let halve l =
    ...
    let rec loop i buff l =
      match l with
      | [] -> ([], [])
      | h :: t -> if i <= n / 2 then loop (i + 1) (h :: buff) t else (buff, l)
    in
    let first_half, second_half = loop 0 [] l in
    ( fst (List.hd first_half)
    , List.rev first_half
    , fst (List.hd (List.rev second_half))
    , second_half )
  ...
  let rec split_buf lb k out1 out2 =
    ...
    | [] -> (out1, out2)
    | (k', v) :: lb' ->
        if Ord.compare k' k <= 0 then split_buf lb' k ((k', v) :: out1) out2
        else split_buf lb' k out1 ((k', v) :: out2)
  ...
  let merge t t' =
    ...
    | Node (lt, lb), Node (lt', lb') ->
        Node (List.append lt lt', List.append lb lb')
    | _ -> assert false
        ...
        let rec loop ldata l =
          ...
          | h :: t -> (
            ...
            | (k, data) :: ldata' ->
                ...
                if c < 0 then (k, data) :: loop ldata' l
                else if c = 0 then (k, data) :: loop ldata' t
                else h :: loop ldata t )
        in
        Leaf (loop ldata l)
    | Node (lt, lb) ->
        let lb =
          List.sort
            (fun (k, _) (k', _) -> Ord.compare k k')
            (List.rev_append ldata lb)
        in
        if needs_flush lb then Node (flush lb lt, []) else Node (lt, lb)
  ...
  and flush lb lt =
    ...
    | _ -> (
      ...
      | (k, t) :: lt' ->
          let lflush, lb' =
            List.partition (fun (kb, _) -> Ord.compare kb k < 0 || lt' = []) lb
          in
          let newk =
            List.fold_left
  ...
  let rec remove_l k lt =
    ...
    | [] -> []
    | (k', t') :: lt' ->
        if Ord.compare k k' < 0 then (k', remove k t') :: lt'
        else (k', t') :: remove_l k lt'

  and remove k t =
    match t with
    | Leaf l -> Leaf (List.remove_assoc k l)
    | Node (lt, lb) ->
        if List.mem_assoc k lb then Node (lt, List.remove_assoc k lb)
        else Node (merge_l (remove_l k lt), lb)

  let rec find_opt k t =
    match t with
    | Leaf l -> List.assoc_opt k l
    | Node (lt, lb) -> (
      try Some (List.assoc k lb) with Not_found ->
        let rec loop lt =
          match lt with
          | [] -> None
  ...
  let find x t =
    match find_opt x t with Some data -> data | None -> raise Not_found
  ...
  let bindings t =
    let rec loop t =
      ...
      | Node (lt, lb) ->
          List.rev_append lb
            (List.flatten (List.map (fun (_, t) -> loop t) lt))
    in
    List.sort (fun (k, _) (k', _) -> Ord.compare k k') (loop t)
  ...
  let to_string t fk fv =
    ...
    let rec loop t tab =
      ...
      | Leaf l ->
          ...
          List.iter
            (fun (k, v) -> Buffer.add_string buff (fk k ^ " : " ^ fv v ^ " ; "))
            l ;
